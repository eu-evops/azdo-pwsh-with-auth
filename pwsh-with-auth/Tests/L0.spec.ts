...
describe('Powershell with Authentication Suite', function () {
  ...
  describe('Docker Registry', function () {
    it('Runs successfully for Docker Registry', (done) => {
      ...
      expect(tr.invokedToolCount).toBe(2); // 1 for task, and two for login and logout
      expect(tr.stderr).toBeEmpty();
      expect(tr.errorIssues).toBeEmpty();
      expect(tr.succeeded).toBeTrue();
      ...
      expect(extractFromLog(tr.stdout, "\\[ENV\\] - DOCKER_CONFIG")).toBe(configPath)

      expect(tr.stdout).toContain(`[command]docker --config ${configPath} logout https://registryUrl`)
    ...
    it('Does not attempt log out if login failed', (done) => {
      let tp = path.join(__dirname, 'DockerRegistryServiceFailingTestFailedLogin.js');
      let tr: ttm.MockTestRunner = new ttm.MockTestRunner(tp);

      tr.run();

      expect(tr.invokedToolCount).toBe(0); // 1 for task, and two for login and logout
      expect(tr.stderr).toBeEmpty();
      expect(tr.errorIssues).toContainValue("Could not log in to Docker registry");
      expect(tr.succeeded).toBeFalse();
